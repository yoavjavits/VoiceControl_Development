...
AudioProcessorWakeWord::AudioProcessorWakeWord(int audio_length, int window_size, int step_size, int pooling_size)
{
    ...
    m_hamming_window = new HammingWindowWakeWord(m_window_size);
}

AudioProcessorWakeWord::~AudioProcessorWakeWord()
{
    free(m_cfg);
    free(m_fft_input);
    free(m_fft_output);
...
void AudioProcessorWakeWord::get_spectrogram_segment(float *output)
{
    // apply the hamming window to the samples
    m_hamming_window->applyWindowWakeWord(m_fft_input);
    // do the fft
    kiss_fftr(
        m_cfg,
        m_fft_input,
...
void AudioProcessorWakeWord::get_spectrogramWakeWord(RingBufferAccessor *reader, float *output_spectrogram)
{
