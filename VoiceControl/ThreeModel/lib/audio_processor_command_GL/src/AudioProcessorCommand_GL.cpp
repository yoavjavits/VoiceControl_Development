...
#include <algorithm>
#include <Arduino.h>
#include "AudioProcessorCommand_GL.h"
#include "HammingWindowCommand_GL.h"
#include "RingBuffer.h"

#define EPSILON 1e-6

AudioProcessorCommand_GL::AudioProcessorCommand_GL(int audio_length, int window_size, int step_size, int pooling_size)
{
    ...
    m_hamming_window = new HammingWindowCommand_GL(m_window_size);
    // track the noise floor
    m_smoothed_noise_floor = 0;
}

AudioProcessorCommand_GL::~AudioProcessorCommand_GL()
{
    free(m_cfg);
    free(m_fft_input);
    free(m_fft_output);
...
void AudioProcessorCommand_GL::get_spectrogram_segment_GL(float *output)
{
    // apply the hamming window to the samples
    m_hamming_window->applyWindowCommand_GL(m_fft_input);
    // do the fft
    kiss_fftr(
        m_cfg,
        m_fft_input,
...
bool AudioProcessorCommand_GL::get_spectrogramCommand_GL(RingBufferAccessor *reader, float *output_spectrogram)
{
    ...
    for (int window_start = startIndex; window_start < startIndex + 16000 - m_window_size; window_start += m_step_size)
    {
        ...
        get_spectrogram_segment_GL(output_spectrogram);
