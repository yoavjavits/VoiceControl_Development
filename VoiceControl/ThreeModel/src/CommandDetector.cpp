...
#include "../lib/audio_processor_command_GL/src/AudioProcessorCommand_GL.h"
#include "../lib/audio_processor_command_FLU/src/AudioProcessorCommand_FLU.h"
#include "../lib/audio_processor_command_BRU/src/AudioProcessorCommand_BRU.h"
...
#include "../lib/neural_network_command_GL/src/NeuralNetworkCommand_GL.h"
...
void CommandDetector::run()
{
    if (isWakeWord)
    {
        ...
        {
            /* Create for WakeWord */
            m_nn_wake_word = new NeuralNetworkCommand_GL();
            // Serial.println("Created Neural Network Wake Word");
            //  create our audio processor
            m_audio_processor_wake_word = new AudioProcessorCommand_GL(AUDIO_LENGTH, WINDOW_SIZE, STEP_SIZE, POOLING_SIZE);
            first_time = false;
        }

        //Serial.println("after neurl network");
        ...
        float *input_buffer = m_nn_wake_word->getInputBufferCommand_GL();

        //Serial.println("after input buffer");

        // process the samples to get the spectrogram
        bool is_valid_wakeword = m_audio_processor_wake_word->get_spectrogramCommand_GL(reader, input_buffer);

        //Serial.println("after spectrogram");

        // finished with the sample reader
        delete reader;

        // get the prediction for the spectrogram
        NNResult_GL output = m_nn_wake_word->predictCommand_GL();

        //Serial.println("after prediction");

        //Serial.println(output.index);
        //Serial.println(output.score);

        long end = millis();

        // use quite a high threshold to prevent false positives
        if (output.score > 0.95 && output.index != 2 && start - m_last_detection > WAIT_PERIOD)
        {
            ...
            case 1:
                Serial.printf("P(%.2f): Detect the word learn\n", output.score);
                delay(300);
                ProcessLearnCommand();
